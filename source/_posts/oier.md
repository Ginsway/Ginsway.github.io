...
### P14635 [NOIP2025] 糖果店
都是贪心想成DP啊，不知道怎么证明正确性。
### P5076 【深基16.例7】普通二叉树（简化版）
递归时没有返回结果了
初始的的子树大小没有初始化
### P3369 【模板】普通平衡树
1. `merge(l,r)`一定是要保证l的数值是小于r的，在递归时要注意l,lp[r]的大小。  
```cpp
} else {
		lp[r] = merge(l,lp[r]);//l一定是小于lp[r]的
		push_up(r);
		return r;
	}
```
```cpp
void insert(int64 k) {
	auto splition = split(root, k);
	root = merge(merge(splition.first, point_sum), splition.second);//填id而不是val
}
```
2.插入式要merge节点id,而不是值k。